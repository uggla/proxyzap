...
def initialize_logger(
    LOGFILE, CONSOLE_LOGGER_LEVEL, FILE_LOGGER_LEVEL, logger_name=None
):
    ...
    try:
        ...
        print(
            "ERROR: {} does not exist or is not writeable.\n".format(LOGFILE)
        )
        print(
            "       Try to create directory {}".format(
                os.path.dirname(LOGFILE)
            )
        )
        print("       using: mkdir -p {}".format(os.path.dirname(LOGFILE)))
        sys.exit(1)
...
def get_gw():
    ...
    cmd = "ip route show"
    route = subprocess.check_output(cmd.split()).decode("utf-8").split("\n")
    gateway = re.search(r"default via (\d+.\d+.\d+.\d+)", route[0])
    if not gateway:
        logger.error(
            "Default gateway not found or "
...
def notify(msg):
    ...
    notif = Notify.Notification.new(
        "proxyzap", msg, "dialog-information"  # titre  # message  # icon
    )
    notif.show()
...
class GnomeProxy(object):
    ...
    def get_proxy_settings(self):
        ...
        self.proxy_mode = str(gsettings.get_value("mode")).replace("'", "")
        self.proxy_ignore = (
            str(gsettings.get_value("ignore-hosts"))
            .replace("'", "")
            .replace("[", "")
            .replace("]", "")
            .split(",")
        ...
        self.proxy_http_url = str(gsettings.get_value("host")).replace("'", "")
        self.proxy_http_port = str(gsettings.get_value("port")).replace(
            "'", ""
        )

        gsettings = Gio.Settings.new("org.gnome.system.proxy.https")
        self.proxy_https_url = str(gsettings.get_value("host")).replace(
            "'", ""
        )
        self.proxy_https_port = str(gsettings.get_value("port")).replace(
            "'", ""
        )

        gsettings = Gio.Settings.new("org.gnome.system.proxy.ftp")
        self.proxy_ftp_url = str(gsettings.get_value("host")).replace("'", "")
        self.proxy_ftp_port = str(gsettings.get_value("port")).replace("'", "")
    ...
    def set_proxy_settings(self, mode):
        ...
        """
        gsettings = Gio.Settings.new("org.gnome.system.proxy")
        gsettings.set_value("mode", GLib.Variant("s", mode))
        ...
        if mode == "manual":
            if self.proxy_ignore != self.PROXYIGNORE:
                ...
                gsettings = Gio.Settings.new("org.gnome.system.proxy")
                gsettings.set_value(
                    "ignore-hosts", GLib.Variant("as", self.PROXYIGNORE)
                )

            if self.proxy_http_url != self.PROXY:
                gsettings = Gio.Settings.new("org.gnome.system.proxy.http")
                gsettings.set_value("host", GLib.Variant("s", self.PROXY))
                gsettings.set_value("port", GLib.Variant("i", self.PROXYPORT))
                gsettings = Gio.Settings.new("org.gnome.system.proxy.https")
                gsettings.set_value("host", GLib.Variant("s", self.PROXY))
                gsettings.set_value("port", GLib.Variant("i", self.PROXYPORT))
                gsettings = Gio.Settings.new("org.gnome.system.proxy.ftp")
                ...
                gsettings.set_value("port", GLib.Variant("i", self.PROXYPORT))
                logger.debug(
                    "Values : PROXY: %s, PORT: %s"
                    % (self.PROXY, self.PROXYPORT)
                )
                logger.debug(
                    "Ignoring proxy for : %s"
                    % (str.join(",", self.PROXYIGNORE))
                )
...
class DnfProxy(object):
    ...
    def get_proxy_settings(self):
        ...
        try:
            ...
            msg = (
                "DNF configuration file %s missing or invalid"
        ...
        if dnfconf.has_section("main"):
            if dnfconf.has_option("main", "proxy"):
                protocol, host, port = (
                    dnfconf.get("main", "proxy").replace("//", "").split(":")
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "Configuration file invalid key "
    ...
    while 1:
        ...
        if SUBGW == gateway:
            ...
            if DNF_PROXY_CONTROL:
                if not dnf_proxy.get_config()["host"] is None:
